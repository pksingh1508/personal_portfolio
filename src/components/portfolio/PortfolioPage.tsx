...
function HeroSection() {
  return (
    <section
      className="relative flex min-h-[921px] items-center overflow-hidden px-10 md:px-20"
      id="hero"
    >
...
function AboutSection() {
  return (
    <section className="relative bg-surface-container px-10 py-32 md:px-20" id="about">
      <div className="grid grid-cols-1 gap-16 md:grid-cols-12">
        ...
        <Reveal className="flex flex-col justify-center md:col-span-8" delay={0.12}>
...
function BuildCard({ build, index }: { build: (typeof builds)[number]; index: number }) {
  return (
    <motion.article
      className={`group relative flex aspect-square cursor-pointer flex-col overflow-hidden border border-outline-variant/10 ${
        build.offset ? "bg-surface-container-high md:translate-y-12" : "bg-surface-container"
    ...
    >
      ...
      <div className="glitch-img-container glitch-overlay absolute inset-0 opacity-10 grayscale transition-all duration-1000 group-hover:scale-110 group-hover:opacity-20">
        <img className="h-full w-full object-cover" src={build.image} alt={build.imageAlt} />
        <div
          className="glitch-effect"
...
function StackSection() {
  return (
    <section className="bg-surface-container-lowest px-10 py-32 md:px-20" id="stack">
      <div className="mx-auto max-w-6xl">
        <div className="flex flex-col items-start gap-12 md:flex-row">
          <Reveal className="md:w-1/3">
            <h2 className="mb-6 font-headline text-5xl font-bold">{stack.title}</h2>
            ...
            <div className="mt-10 border-l-2 border-primary bg-surface-container p-6">
              ...
              <span className="font-headline text-lg font-bold">{stack.focus}</span>
            </div>
          </Reveal>
...
function TimelineSection() {
  return (
    <section className="relative overflow-hidden px-10 py-32 md:px-20" id="logs">
      <motion.div
      ...
      <div className="relative z-10 mx-auto max-w-4xl space-y-24">
        {timeline.map((entry, index) => (
          ...
          >
            <div className={`md:w-1/2 ${entry.reverse ? "md:text-left" : "md:text-right"}`}>
            ...
            >
              <Symbol className={entry.active ? "text-on-primary" : "text-surface"}>
                {entry.icon}
              </Symbol>
            </div>
  ...
  return (
    <section className="relative px-10 py-32 md:px-20" id="lab">
      <div className="pointer-events-none absolute inset-0 z-0 overflow-hidden">
        <div className="absolute -left-20 -top-20 h-[600px] w-[600px] bg-primary/5 blur-[120px]" />
      </div>

      <div className="relative z-10">
        <div className="mb-16 flex items-center gap-6">
          <h2 className="font-headline text-5xl font-bold">NOW_COMPILING</h2>
          <div className="h-[2px] flex-1 bg-outline-variant opacity-20" />
        </div>
        ...
        <div className="grid grid-cols-1 gap-10 md:grid-cols-2">
          {labs.map((lab, index) => (
            ...
            >
              ...
              <h3 className="mb-4 font-headline text-3xl font-bold">{lab.title}</h3>
